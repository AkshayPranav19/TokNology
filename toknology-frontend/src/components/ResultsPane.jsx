...
export default function ResultsPane({ regions = [], result }) {
  ...
  // ---- Dynamic citations logic ----
  const backendCitations = Array.isArray(d?.citations) ? d.citations : [];
  const fallbackUrls = [
  ...
  // Use backend citations if available, otherwise use fallback URLs
  const citationsToShow = backendCitations.length > 0 ? backendCitations : [];
  
  // Only show citations if we have backend data (decision exists) and either citations or rationale
  const shouldShowCitations = d && (citationsToShow.length > 0 || rationale.trim());
  
  const citationNodes = shouldShowCitations && citationsToShow.length > 0 ? 
    citationsToShow.map((citation, idx) => {
      // Handle both string URLs and citation objects
      const url = typeof citation === 'string' ? citation : citation?.url || citation?.link;
      const title = typeof citation === 'object' ? citation?.title || citation?.name : null;
    ...
    <Card className="p-4">
      ...
      {shouldShowCitations && (
        <div className="mt-4">
          ...
          <div className="mt-2 space-y-2">
            <div className="rounded-lg border border-zinc-200 p-3 text-sm bg-zinc-50 text-zinc-700">
              {rationale.trim() || "Analysis complete - see citations below."}
            </div>
      ...
      {/* Show placeholder when no backend data yet */}
      {!d && (
        <div className="mt-4">
          <p className="text-sm font-medium">Why (with citations)</p>
          <div className="mt-2 space-y-2">
